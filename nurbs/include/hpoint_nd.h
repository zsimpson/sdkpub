...
/*  template <>
  ...
  // An attempt to speed up the object.  Didn't quite work
  template <>
  struct HPoint_nD<double,3> {
    typedef double T;
    T locdata[4] ; 
	...
	HPoint_nD() : data(locdata), created(0) { x() = y() = z() = w() = T(); }
	HPoint_nD(T *d,int c) : data(d), created(c) { } 
	HPoint_nD(T a) : data(locdata), created(0) { x() = y() = z() = w() = a; }
	HPoint_nD(T X, T Y, T Z, T W) : data(locdata), created(0) { x()=X ; y()=Y; z()=Z; w()=W;}
	HPoint_nD(const HPoint_nD& a) : data(locdata), created(0) { x()=a.x(); y()=a.y(); z()=a.z(); w()=a.w(); }
	HPoint_nD(const Point_nD<T,3>& a) : data(locdata), created(0) { x()=a.x(); y()=a.y(); z()=a.z();  w() = T(1); }
    ~HPoint_nD() { if (created) delete []data ; }
  ...
  */
  ...
  inline HPoint_nD<float,3> operator*( double a, const HPoint_nD<float,3>&b) {
    HPoint_nD<float,3> mul((float)(b.x()*a),(float)(b.y()*a),(float)(b.z()*a),(float)(b.w()*a)) ;
    return mul;
  }
  ...
  inline HPoint_nD<float,2> operator*(double a, const HPoint_nD<float,2>&b) {
    HPoint_nD<float,2> mul((float)(b.x()*a),(float)(b.y()*a),(float)(b.w()*a)) ;
    return mul;
  }
  ...
  inline T distance3D(const HPoint_nD<T,D>& a, const HPoint_nD<T,D>& b) { 
    return (T)norm(project(a)-project(b)) ;
  }
