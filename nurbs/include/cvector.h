...
namespace PLib {
  ...
  */
  ...
  public:
    ...
    T& operator[](const int i) { return BasicArray<T>::x[i%BasicArray<T>::sze]; }
    T  operator[](const int i) const   { return BasicArray<T>::x[i%BasicArray<T>::sze]; }
    
    void put(T v) { BasicArray<T>::x[index] = v ; index = (index+1)%BasicArray<T>::sze; }
