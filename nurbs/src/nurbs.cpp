...
NurbsSurface<T,3> NurbsCurve<T,N>::drawAaImg(Image_Color& Img, const Color& color, const NurbsCurve<T,3>& profile, const NurbsCurve<T,3>& scaling, int precision, int alpha){
  ...
  T distance = (T)(norm(b-a) + norm(c-b)) ;
  ...
  distance = (T)(norm(b2-a2) + norm(c2-b2)) ;
...
HPoint_nD<T,N> NurbsCurve<T,N>::operator()(T u) const{
  ...
  for(int i=deg_;i>=0;--i) {
	  const HPoint_nD<T,N> *tmpPt2 = P.getPtr(span-deg_+i);
	  tmpPt = *tmpPt2;
	  tmpPt *= Nb[i];
	  p += tmpPt;
...
void NurbsCurve<T,N>::removeKnotsBound(const Vector<T>& ub,
  ...
  while(1) {
	...
	NewError[i] = (T)((1.0-a)*Br[r] * basisFun(u,r-k+1));
      }
      else{
	u = ub[i] ;
	k = (deg_+s)/2 ;
...
T chordLengthParam(const Vector< Point_nD<T,N> >& Q, Vector<T> &ub){
  ...
  if(d>0){
    for(i=1;i<ub.n()-1;++i){
      ub[i] = (T)(ub[i-1] + norm(Q[i]-Q[i-1])/d) ;
    }
...
void NurbsCurve<T,N>::projectTo(const Point_nD<T,N>& p, T guess, T& u, Point_nD<T,N>& r, T e1, T e2,int maxTry) const{
  ...
  while(1) {
    ...
    c2 = (T)(norm((Point_nD<T,N>)(cd*(c-p)))) ;
    //c2 *= c2 ;
    c2 /= (T)(norm(cd)*norm(c-p)) ;
    //if(c2<e2*e2){
    if(c2<e2){
      r =c ;
      return ;
    ...
    un = (T)(u- cd*(c-p)/(cdd*(c-p)+norm2(cd))) ;
  ...
  default:
    {
      ...
      for(i=0;i<ub.n()-1;++i){
	ub2[2*i] = ub[i] ;
	ub2[2*i+1] = (T)((ub[i]+ub[i+1])/2.0) ;
      }
      
      ub2[ub2.n()-2] = (T)((ub2[ub2.n()-1]+ub2[ub2.n()-3])/2.0) ;
...
int NurbsCurve<T,N>::movePoint(T u, const BasicArray< Point_nD<T,N> >& delta) {
  ...
  for(i=0;i<m;++i){
    P[span-deg_+i].x() += (T)(dP(i,0)*P[span-deg_+i].w()) ;
    P[span-deg_+i].y() += (T)(dP(i,1)*P[span-deg_+i].w()) ;
    P[span-deg_+i].z() += (T)(dP(i,2)*P[span-deg_+i].w()) ;
  }
...
int NurbsCurve<T,N>::movePoint(const BasicArray<T>& ur, const BasicArray< Point_nD<T,N> >& D, const BasicArray_INT& Dr, const BasicArray_INT& Dk, const BasicArray_INT& fixCP) {
  ...
  for(i=0;i<map.n();++i){
    P[map[i]].x() += (T)(dP(i,0)*P[map[i]].w()) ;
    P[map[i]].y() += (T)(dP(i,1)*P[map[i]].w()) ;
    P[map[i]].z() += (T)(dP(i,2)*P[map[i]].w()) ;
  }
...
BasicList<Point_nD<T,N> > NurbsCurve<T,N>::tesselate(T tolerance,BasicList<T> *uk) const {
  ...
  if(ca.n()==1){
    ...
    int n = (int)rint((float)(sqr2*norm(maxD)/tolerance)) ;
    ...
    for(int i=0;i<ca.n();++i){
      ...
      //      list2.addElements(ca[i].tesselate(tolerance,uk));
...
T chordLengthParamClosed(const Vector< Point_nD<T,N> >& Qw, Vector<T> &ub,int deg){
  ...
  for(i=1;i<=ub.n()-deg;i++){
    d += (T)(norm(Qw[i]-Qw[i-1])) ;
  }
  if(d>0){
    for(i=1;i<ub.n();++i)
      ub[i] = (T)(ub[i-1] + norm(Qw[i]-Qw[i-1]));    
    // Normalization
    for(i=0;i<ub.n();++i)
      ub[i]/=  d;
  }
...
int NurbsCurve<T,N>::writeDisplayLINE(const char* filename,const Color& color, int iNu,T u_s, T u_e) const 
{
  ...
  float fR=(float)(color.r/255.0);
  float fG=(float)(color.g/255.0);
  float fB=(float)(color.b/255.0);
  float fA=1;
  /* Colour flag = ONE_COLOUR */
  fout << 0 << ' ' ;
  /* The colour */
