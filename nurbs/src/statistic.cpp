...
namespace PLib {
  ...
  template <class T>
	...
	error.fatal();
      ...
      if(n<=1) return 0;
      if(n <= 100) 
	return (T)(a[n] ? a[n] : (a[n] = (T)lnOfGamma(n+1.0))) ;
      return (T)lnOfGamma(n+1.0) ; // out of range of table
    }
...
//      T gamser, gammcf, gln ;
	...
	return gammaSerie(a,x,gln);
      ...
      return (T)(1.0-gammaSerieCF(a,x,gln));
    }
...
//      T gamser, gammcf, gln ;
	...
	error.fatal();
      ...
      if(x < (a+1)){
	return (T)(1.0-gammaSerie(a,x,gln));
      }
  ...
  template <class T>
    T errorFcnChebyshevC(T x)
    {
      ...
      z = absolute(x);
      t = T(1)/(T)((T(1)+0.5*z)) ;
      ans=(T)(t*exp(-z*z-1.26551223+t*(1.00002368+t*(0.37409196+t*0.09678418+
  ...
  template <class T>
	...
	for(k=j+1;k<n;++k){
      ...
      z = tau/T(sqrt(svar)) ;
      prob = errorFcnChebyshevC<T>((T)(absolute(z)/1.4142136)) ;
    }
  ...
  void kendallTau(const BasicArray<int>& data1, const BasicArray<int>& data2, float &tau, float &z, float& prob)
    {
      ...
      for(j=0;j<n-1;++j)
	for(k=j+1;k<n;++k){
	  a1= (float)(data1[j]-data1[k]) ;
	  a2= (float)(data2[j]-data2[k]) ;
	  aa=a1*a2;
	  if(aa){
	    ++n1 ; //no tie
	    ++n2 ;
      ...
      prob = errorFcnChebyshevC<float>(absolute(z)/1.4142136f) ;
    }
