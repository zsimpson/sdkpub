...
#else
      Error error("invm");
      error << "matrix inverse, not square: " << rows() << " by " << this->cols() << endl;
      error.fatal();
#endif
    }
...
double SVDMatrix<T>::left_householder(Matrix<T>& A, const int i)
 {
   ...
   const float beta = (float)(-A(i,i)*new_Aii + Norm_sqr);
   A(i,i) -= (T)new_Aii;			// UPi[i] = A[i,i] - (-Norm)
...
double SVDMatrix<T>::right_householder(Matrix<T>& A, const int i)
{
   ...
   const float beta = (float)(-A(i,i+1)*new_Aii1 + Norm_sqr);
   A(i,i+1) -= (T)new_Aii1;		// VPi[i+1] = A[i,i+1] - (-Norm)
...
double SVDMatrix<T>::bidiagonalize(Vector<T>& super_diag, const Matrix<T>& _A)
{
  ...
  for(int i=0; i<N; i++)
  {
    const T& diagi = sig_[i] = (T)left_householder(A,i);
    if( i < N-1 )
      super_diag[i+1] = (T)right_householder(A,i);
		   ...
		   const double cos_ph, const double sin_ph)
{
  for(int l=0; l<Mu.rows(); l++)
  {
    ...
    const T Ujl_was = Ujl;
    Ujl =  (T)(cos_ph*Ujl_was + sin_ph*Uil);
    Uil = (T)(-sin_ph*Ujl_was + cos_ph*Uil);
  }
}
...
void SVDMatrix<T>::rip_through(Vector<T>& super_diag, const int k, const int l, const double eps)
{
  ...
  for(int i=l; i<=k; i++)
  {
    ...
    super_diag[i] *= (T)cos_ph;
    if( absolute(f) <= eps )
      break;			// Current J[l-1,l] became unsignificant
    cos_ph = sig[i]; sin_ph = -f;	// unnormalized sin/cos
    const double norm = (T)(sig_[i] = hypot(cos_ph,sin_ph)); // sqrt(sin^2+cos^2)
  ...
  for(register int k=N-1; k>=0; k--)	// QR-iterate upon J[l:k,l:k]
  {
    ...
    while(absolute(super_diag[k]) > eps)// until superdiag J[k-1,k] becomes 0
    {
      ...
      for(register int i=l+1; i<=k; i++)
      {
      	...
      	sin_th *= (T)Ji1i; Ji1i *= (T)cos_th; cos_th = (T)shift;
        ...
        Ji1i1 = (T)(-sin_ph*Ji1i + cos_ph*Jii);	// New Jii, would carry over
