    ...
    int refreshrate )
{
	...
	// ZBS changed to eliminate the failure check for multi-window processing
    RegisterClass( &wc );
		// New Code doesn't check error
		// Old code does:
/*
    if( !RegisterClass( &wc ) )
    {
...
// Function added by ZBS 19 Apr 2004
void _glfwPlatformGetWindowGeom( int *x, int *y, int *w, int *h ) {
	int a, b, c;
	int style;
	RECT rect;
	GetWindowRect( _glfwWin.Wnd, &rect );

	style = GetWindowLong( _glfwWin.Wnd, GWL_STYLE );
	if( style & WS_POPUP ) {
		*x = rect.left;
		*y = rect.top;
