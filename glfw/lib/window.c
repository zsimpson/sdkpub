...
//========================================================================
// ZBS extensions and additions
//========================================================================

_GLFWwin aliases[MAX_ALIASES] = {0,};

GLFWAPI int GLFWAPIENTRY glfwZBSExt_GetHWND()
{
	#ifdef WIN32
	return (int)_glfwWin.Wnd;
	#endif
}

GLFWAPI int GLFWAPIENTRY glfwZBSExt_CreateWindowAlias( int alias )
{
	if( alias >= 0 && alias < MAX_ALIASES ) {
			if( ! aliases[alias].Opened ) {
					memcpy( &aliases[alias], &_glfwWin, sizeof(_glfwWin) );
					return 1;
			}
	}
	return 0;
...
GLFWAPI int GLFWAPIENTRY glfwZBSExt_SetWindow( int alias )
{
	#ifdef WIN32
	if( alias >= 0 && alias < MAX_ALIASES ) {
			if( aliases[alias].Opened ) {
					memcpy( &_glfwWin, &aliases[alias], sizeof(_glfwWin) );
					wglMakeCurrent( _glfwWin.DC, _glfwWin.RC );
					return 1;
			}
	}
	#endif
...
GLFWAPI int GLFWAPIENTRY glfwZBSExt_MakeCurrent( int alias )
{
	#ifdef WIN32
	if( alias >= 0 && alias < MAX_ALIASES ) {
			if( aliases[alias].Opened ) {
...
static GLFWmutex zbsLock = 0;

GLFWAPI void GLFWAPIENTRY glfwZBSExt_LockWindow( int alias )
{
	if( !zbsLock ) {
			zbsLock = glfwCreateMutex();
	}
	glfwLockMutex( zbsLock );
...
GLFWAPI void GLFWAPIENTRY glfwZBSExt_UnlockWindow( int alias )
{
	if( alias >= 0 && alias < MAX_ALIASES ) {
			if( aliases[alias].Opened ) {
					memcpy( &_glfwWin, &aliases[alias], sizeof(_glfwWin) );
//						wglMakeCurrent( _glfwWin.DC, _glfwWin.RC );
					glfwUnlockMutex( zbsLock );
			}
	}
}

GLFWAPI void GLFWAPIENTRY glfwGetWindowGeom( int *x, int *y, int *w, int *h )
{
	if( x ) *x = 0;
	if( y ) *y = 0;
	if( w ) *w = 0;
	...
	_glfwPlatformGetWindowGeom( x, y, w, h );
}
