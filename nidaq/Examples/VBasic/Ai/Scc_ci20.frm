...
Sub cmdDoOperation_Click()
    ...
    Static pdCurrentBuffer(30) As Double
...
'     The readings are scaled internally in DAQ_VScale to current values
'      (in mA), provided that you have configured your SCC-CI20 in the
'      NI-DAQ Configuration Utility.  If you have not configured your
'      SCC-CI20 in the NI-DAQ Configuration Utility, the scaled data
'      will be off by a factor of 0.249.

    iStatus% = DAQ_VScale(iDevice%, iChan%, iGain%, dGainAdjust#, dOffset#, ulCount&, piBuffer%(0), pdCurrentBuffer#(0))

    iRetVal% = NIDAQErrorHandler(iStatus%, "DAQ_VScale", iIgnoreWarning%)

    iRetVal% = NIDAQMean(pdCurrentBuffer#(0), ulCount&, WFM_DATA_F64, dAvgCurrent#)

    Call PrintText(txtStatusBox, "The plot shows all channel data in order. ")

'     Plot acquired data

    iRetVal% = NIDAQPlotWaveform(pdCurrentBuffer#(0), ulCount&, WFM_DATA_F64)

    Call PrintText(txtStatusBox, "The average current reading is " +  Trim$(Str$(dAvgCurrent#)) + "mA. ")
