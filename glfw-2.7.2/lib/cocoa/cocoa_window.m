...
// Function added by ZBS 19 Apr 2004; ported to GLFW2.7.2 by TFB
void _glfwPlatformGetWindowGeom( int *x, int *y, int *w, int *h ) {
	NSRect frameRect = [_glfwWin.window frame];
...
// NOTE: TFB modified this function to simply set the frame origin because
// I don't care about what screen it's on, I just want to use a single coordinate
// system which implicitly lands the window on whichever screen is appropratiate, so
// my version is the single line below, and the previous code from GLFW2.7.2 is 
// commented out in its original location above in this file.
void _glfwPlatformSetWindowPos( int x, int y )
{
	[_glfwWin.window setFrameOrigin:NSMakePoint( x, y ) ];
}
