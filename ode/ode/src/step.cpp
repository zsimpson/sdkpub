...
// ZBS removed so that everything in here could be replaced with ZBSALLOCA
...
//  dxJoint **joint = (dxJoint**) ALLOCA (nj * sizeof(dxJoint*));
  ZBSALLOCA( dxJoint *, joint, nj * sizeof(dxJoint*) );
...
//  dReal *I = (dReal*) ALLOCA (3*nb*4 * sizeof(dReal));
  ZBSALLOCA( dReal, I, 3*nb*4 * sizeof(dReal) );
//  dReal *invI = (dReal*) ALLOCA (3*nb*4 * sizeof(dReal));
  ZBSALLOCA( dReal, invI, 3*nb*4 * sizeof(dReal) );
...
//  dxJoint::Info1 *info = (dxJoint::Info1*) ALLOCA (nj*sizeof(dxJoint::Info1));
  ZBSALLOCA( dxJoint::Info1, info, nj*sizeof(dxJoint::Info1) );
//  int *ofs = (int*) ALLOCA (nj*sizeof(int));
  ZBSALLOCA( int, ofs, nj*sizeof(int) );
...
//  dReal *invM = (dReal*) ALLOCA (n6*nskip*sizeof(dReal));
  ZBSALLOCA( dReal, invM, n6*nskip*sizeof(dReal) );
...
//  dReal *fe = (dReal*) ALLOCA (n6 * sizeof(dReal));
  ZBSALLOCA( dReal, fe, n6 * sizeof(dReal) );
//  dReal *v = (dReal*) ALLOCA (n6 * sizeof(dReal));
  ZBSALLOCA( dReal, v, n6 * sizeof(dReal) );
  //dSetZero (fe,n6);
  //dSetZero (v,n6);
  for (i=0; i<nb; i++) {
...
//  dReal *vnew = (dReal*) ALLOCA (n6 * sizeof(dReal));
  ZBSALLOCA( dReal, vnew, n6 * sizeof(dReal) );
  dSetZero (vnew,n6);
...
//    dReal *c = (dReal*) ALLOCA (m*sizeof(dReal));
    ZBSALLOCA( dReal, c, m*sizeof(dReal) );
//    dReal *cfm = (dReal*) ALLOCA (m*sizeof(dReal));
    ZBSALLOCA( dReal, cfm, m*sizeof(dReal) );
//    dReal *lo = (dReal*) ALLOCA (m*sizeof(dReal));
    ZBSALLOCA( dReal, lo, m*sizeof(dReal) );
//    dReal *hi = (dReal*) ALLOCA (m*sizeof(dReal));
    ZBSALLOCA( dReal, hi, m*sizeof(dReal) );
...
//    dReal *J = (dReal*) ALLOCA (m*nskip*sizeof(dReal));
    ZBSALLOCA( dReal, J, m*nskip*sizeof(dReal) );
    dSetZero (J,m*nskip);
    dxJoint::Info2 Jinfo;
    Jinfo.rowskip = nskip;
...
//    dReal *lambda = (dReal*) ALLOCA (m * sizeof(dReal));
    ZBSALLOCA( dReal, lambda, m * sizeof(dReal) );
//    dReal *residual = (dReal*) ALLOCA (m * sizeof(dReal));
    ZBSALLOCA( dReal, residual, m * sizeof(dReal) );
...
//  dReal *cforce = (dReal*) ALLOCA (nb*8 * sizeof(dReal));
  ZBSALLOCA( dReal, cforce, nb*8 * sizeof(dReal));
...
//    dReal *J = (dReal*) ALLOCA (2*m*8*sizeof(dReal));
    ZBSALLOCA( dReal, J, 2*m*8*sizeof(dReal));
    dSetZero (J,2*m*8);
    dxJoint::Info2 Jinfo;
    Jinfo.rowskip = 8;
    Jinfo.fps = stepsize1;
...
//    dReal *JinvM = (dReal*) ALLOCA (2*m*8*sizeof(dReal));
    ZBSALLOCA( dReal, JinvM, 2*m*8*sizeof(dReal));
...
//    dReal *tmp1 = (dReal*) ALLOCA (nb*8 * sizeof(dReal));
    ZBSALLOCA( dReal, tmp1, nb*8 * sizeof(dReal));
    //dSetZero (tmp1,nb*8);
