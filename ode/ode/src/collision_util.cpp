...
// ZODEStackMem
// Created by ZBS in order to fix the ALLOCA overflow problems
// I had to create another macro which would allow for
// memory to be allocated on a different stack
#include "assert.h"
static char *ZODEStackMem_buf = 0;
static char *ZODEStackMem_tail = 0;
static char *ZODEStackMem_ptr = 0;
ZODEStackMem::ZODEStackMem( int n ) {
	if( !ZODEStackMem_buf ) {
		int allocSize = 4096 * 512;
		ZODEStackMem_buf = (char *)malloc( allocSize );
		ZODEStackMem_buf += 16 - ((unsigned int)ZODEStackMem_buf & 15);
		ZODEStackMem_tail = &ZODEStackMem_buf[allocSize - 16];
		ZODEStackMem_ptr = ZODEStackMem_buf;
	}
	ptr = ZODEStackMem_ptr;
	size = n + 16 - (n & 15);
	ZODEStackMem_ptr += size;
	assert( ZODEStackMem_ptr < ZODEStackMem_tail );
}

ZODEStackMem::~ZODEStackMem() {
	ZODEStackMem_ptr -= size;
	assert( ZODEStackMem_ptr >= ZODEStackMem_buf );
}
