...
void CdHardware::OnOK() 
{
	...
	controller_ok =  PICM_CreateController(DC131,
                                           ccdtype, 
                                           readouttype,
                                           APP_NORMAL_IMAGING,
                                           &error_code); 
	if (controller_ok)
	{
		...
		if( (error_code != 0) || !controller_ok )
			...
			CMainFrame::m_efID_EXPERIMENT_FOCUSINGSETUP = TRUE;
			}
	}
	WriteDefaults( expos, luttype, ccdtype, interfacetype, 
...
{
	ReadDefaults( &expos, &luttype, &ccdtype, &interfacetype, 
                    &ioaddtype, &interrupttype,
					&adctype, &readouttype, &versiontype );
