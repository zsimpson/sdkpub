...
HardwareSetup::HardwareSetup(CWnd* pParent /*=NULL*/)
	: CDialog(HardwareSetup::IDD, pParent)
{
	//{{AFX_DATA_INIT(HardwareSetup)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}


void HardwareSetup::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(HardwareSetup)
		// NOTE: the ClassWizard will add DDX and DDV calls here
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(HardwareSetup, CDialog)
	//{{AFX_MSG_MAP(HardwareSetup)
	ON_CBN_SELCHANGE(IDC_CONTROL, OnSelchangeControl)
	ON_BN_CLICKED(IDC_ADCFAST, OnAdcfast)
	ON_BN_CLICKED(IDC_ADCSLOW, OnAdcslow)
	ON_BN_CLICKED(IDC_FULLFRAME, OnFullframe)
	ON_BN_CLICKED(IDC_FRAMETRANSFER, OnFrametransfer)
	ON_CBN_SELCHANGE(IDC_CCD, OnSelchangeCcd2)
	ON_CBN_SELCHANGE(IDC_INTERFACE, OnSelchangeInterface)
	ON_CBN_SELCHANGE(IDC_IOADDR, OnSelchangeIoaddr)
	ON_CBN_SELCHANGE(IDC_INTERUPT, OnSelchangeInterupt)
	ON_CBN_SELCHANGE(IDC_CONTROLVER, OnSelchangeControlver)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// HardwareSetup message handlers


BOOL HardwareSetup::OnInitDialog()
{
...
void HardwareSetup::OnSelchangeControl() 
{
		int ctype;
...
void HardwareSetup::OnAdcfast() 
{
        adctype = 1;	
}

void HardwareSetup::OnAdcslow() 
{
		adctype = 0;
	
}

void HardwareSetup::OnFullframe() 
{
    readouttype = ROM_FULL;
	
}

void HardwareSetup::OnFrametransfer() 
{
	readouttype = ROM_FRAME_TRANSFER;
	
}

void HardwareSetup::OnSelchangeCcd2() 
{
...
void HardwareSetup::OnSelchangeInterface() 
{
	int itype;
...
void HardwareSetup::OnSelchangeIoaddr() 
{
	int iotype;

	iotype = ((CComboBox*)GetDlgItem(IDC_IOADDR))->GetCurSel();
	switch (iotype)
		{
		case 0:					// 0x700
			ioaddtype = 0x700;
...
void HardwareSetup::OnSelchangeInterupt() 
{
	int irqtype;

	irqtype = ((CComboBox*)GetDlgItem(IDC_INTERUPT))->GetCurSel();
	switch (irqtype)
		{
		case 0:					// IRQ 10
...
void HardwareSetup::OnOK() 
{

	unsigned int error_code = 0;
	int controller_ok;

	// create controller and assign detector
	...
	if (controller_ok)
	{
		...
		// set menu items to become enabled 
        	CMainFrame::m_efID_EXPERIMENT_EXPOSURE = TRUE;
...
void HardwareSetup::SetDefaults()
{
...
void HardwareSetup::OnSelchangeControlver() 
{
	int vertype;
