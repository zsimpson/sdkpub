...
BOOL CdHardware::OnInitDialog()
{
	...
	Controller_ComboBox().SetItemData(0, DC131);
	Controller_ComboBox().SetItemData(1, ST133);
	Controller_ComboBox().SetItemData(2, ST133);
    Controller_ComboBox().SetItemData(3, ST133_5MHZ);
	...
	ControlVer_ComboBox().SetItemData(0, 1);
	ControlVer_ComboBox().SetItemData(1, 2);
	ControlVer_ComboBox().SetItemData(2, 3);
	ControlVer_ComboBox().SetItemData(3, 4);
	ControlVer_ComboBox().SetItemData(3, 5);
	SetDefaults();
...
void CdHardware::OnSelchangeControl() 
{
	...
	switch (ctype)
	{
		...
		case 2:					// SpectraMAX
			...
			break;
        case 3:
            controltype = ST133_5MHZ; // 5Mhz ST133
            break;
	}	
}
...
void CdHardware::OnOK() 
{
	...
	if (controller_ok)
	{
		...
		CMainFrame::m_efID_EXPERIMENT_FOCUSSETUP = TRUE;
