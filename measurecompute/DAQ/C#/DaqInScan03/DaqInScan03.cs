...
namespace DaqInScan01
{
	public class frmDataDisplay : System.Windows.Forms.Form
	{
		...
		const int ChanCount = 6;    //  Number of channels in scan
		const int TcChanCount = 2;  // Number of thermocouple channels
		const int NumPoints = 10;   //  Number of data points to collect
		const int NumElements = ChanCount * NumPoints;
		const int NumTCElements  = TcChanCount * NumPoints;

		MccDaq.TempScale CBScale;
		...
		private float[] TempValArray = new float[NumTCElements];
		...
		private void InitializeComponent()
		{
			...
			this.lblDemoFunction.Text = "Demonstration of MccDaq.MccBoard.DaqInScan() and MccDaq.MccBoard.GetTCValue()";
